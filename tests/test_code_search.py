...
from codenav.code_search import CodeSearcher, SearchResult
...
class TestCheckStaleFiles:
    ...
    def test_check_stale_files_returns_dict(self, searcher, sample_codenav, tmp_path):
        """Test that check_stale_files returns proper structure."""
        # Create temp files matching the codenav
        (tmp_path / "src").mkdir()
        ...
        # Update codenav root to temp path
        sample_codenav["root"] = str(tmp_path)
        map_file = tmp_path / ".codenav.json"
        map_file.write_text(json.dumps(sample_codenav))
    ...
    def test_check_stale_files_detects_changes(self, sample_codenav, tmp_path):
    ...
    def test_check_stale_files_detects_missing(self, sample_codenav, tmp_path):
