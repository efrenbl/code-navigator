...
from codenav.code_navigator import add_map_arguments, run_map
from codenav.code_search import add_search_arguments, run_search
from codenav.line_reader import add_read_arguments, run_read
...
class TestCLIHelp:
    ...
    def test_main_help(self):
        ...
        with patch.object(sys, "argv", ["codenav", "--help"]):
    ...
    def test_main_version(self):
        ...
        with patch.object(sys, "argv", ["codenav", "--version"]):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 0
    ...
    def test_map_help(self):
        ...
        with patch.object(sys, "argv", ["codenav", "map", "--help"]):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 0
    ...
    def test_search_help(self):
        ...
        with patch.object(sys, "argv", ["codenav", "search", "--help"]):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 0
    ...
    def test_read_help(self):
        ...
        with patch.object(sys, "argv", ["codenav", "read", "--help"]):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 0
    ...
    def test_stats_help(self):
        ...
        with patch.object(sys, "argv", ["codenav", "stats", "--help"]):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 0
    ...
    def test_no_command_shows_help(self, capsys):
        ...
        with patch.object(sys, "argv", ["codenav"]):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 0
...
class TestAddArgumentsFunctions:
    ...
    def test_add_map_arguments(self):
        ...
        assert args.output == ".codenav.json"
...
class TestMapCommand:
    ...
    def test_run_map_creates_codenav(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            parser = argparse.ArgumentParser()
            add_map_arguments(parser)
            args = parser.parse_args([tmpdir, "-o", "test.codenav.json"])
            ...
            output_path = Path(tmpdir) / "test.codenav.json"
...
class TestSearchCommand:
    ...
    @pytest.fixture
    def codenav_file(self, tmp_path):
    ...
    def test_run_search_finds_symbol(self, codenav_file, capsys):
        """Test that search finds symbols correctly."""
        parser = argparse.ArgumentParser()
        add_search_arguments(parser)
        args = parser.parse_args(["hello", "-m", str(codenav_file)])
    ...
    def test_run_search_with_type_filter(self, codenav_file, capsys):
        """Test that type filter works correctly."""
        parser = argparse.ArgumentParser()
        add_search_arguments(parser)
        args = parser.parse_args(["-t", "class", "-m", str(codenav_file)])
    ...
    def test_run_search_stats(self, codenav_file, capsys):
        """Test that stats command works correctly."""
        parser = argparse.ArgumentParser()
        add_search_arguments(parser)
        args = parser.parse_args(["--stats", "-m", str(codenav_file)])
...
class TestStatsCommand:
    ...
    def test_stats_via_unified_cli(self, tmp_path, capsys):
        ...
        with patch.object(sys, "argv", ["codenav", "stats", "-m", str(map_path)]):
            main()
...
class TestBackwardCompatibility:
    ...
    def test_code_navigator_main_import(self):
        """Test that code_navigator.main is still importable."""
        from codenav.code_navigator import main

        assert callable(main)

    def test_code_search_main_import(self):
        """Test that code_search.main is still importable."""
        from codenav.code_search import main

        assert callable(main)

    def test_line_reader_main_import(self):
        """Test that line_reader.main is still importable."""
        from codenav.line_reader import main
