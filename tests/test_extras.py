...
from codenav.completions import (
    generate_bash_completion,
    generate_zsh_completion,
    get_symbols_from_map,
)
from codenav.exporters import (
    GraphVizExporter,
    HTMLExporter,
    MarkdownExporter,
    get_exporter,
)
from codenav.watcher import CodenavWatcher


@pytest.fixture
def sample_codenav(tmp_path):
    ...
    map_path.write_text(json.dumps(codenav))
    return str(map_path)
...
class TestCompletions:
    ...
    def test_generate_bash_completion(self):
        ...
        assert "_codenav_completions" in script
        assert "complete -F _codenav_completions codenav" in script
    ...
    def test_generate_zsh_completion(self):
        ...
        assert "#compdef codenav" in script
        assert "_codenav()" in script
        assert "map:" in script
        assert "search:" in script
        assert "export:" in script
        assert "watch:" in script

    def test_get_symbols_from_map(self, sample_codenav):
        """Test extracting symbols from a code map."""
        symbols = get_symbols_from_map(sample_codenav)
    ...
    def test_get_symbols_limit(self, sample_codenav):
        """Test symbol extraction with limit."""
        symbols = get_symbols_from_map(sample_codenav, limit=2)
...
class TestMarkdownExporter:
    ...
    def test_markdown_export(self, sample_codenav):
        """Test basic Markdown export."""
        exporter = MarkdownExporter(sample_codenav)
    ...
    def test_markdown_has_symbol_index(self, sample_codenav):
    ...
    def test_markdown_export_to_file(self, sample_codenav, tmp_path):
...
class TestHTMLExporter:
    ...
    def test_html_export(self, sample_codenav):
        """Test basic HTML export."""
        exporter = HTMLExporter(sample_codenav)
    ...
    def test_html_has_styles(self, sample_codenav):
    ...
    def test_html_has_javascript(self, sample_codenav):
...
class TestGraphVizExporter:
    ...
    def test_graphviz_export(self, sample_codenav):
        """Test basic GraphViz export."""
        exporter = GraphVizExporter(sample_codenav)
        dot = exporter.export()
    ...
    def test_graphviz_has_nodes(self, sample_codenav):
    ...
    def test_graphviz_has_clusters(self, sample_codenav):
...
class TestGetExporter:
    ...
    def test_get_markdown_exporter(self, sample_codenav):
        """Test getting Markdown exporter."""
        exporter = get_exporter("markdown", sample_codenav)
        assert isinstance(exporter, MarkdownExporter)

    def test_get_md_exporter(self, sample_codenav):
        """Test getting Markdown exporter with 'md' alias."""
        exporter = get_exporter("md", sample_codenav)
        assert isinstance(exporter, MarkdownExporter)

    def test_get_html_exporter(self, sample_codenav):
        """Test getting HTML exporter."""
        exporter = get_exporter("html", sample_codenav)
        assert isinstance(exporter, HTMLExporter)

    def test_get_graphviz_exporter(self, sample_codenav):
        """Test getting GraphViz exporter."""
        exporter = get_exporter("graphviz", sample_codenav)
        assert isinstance(exporter, GraphVizExporter)

    def test_get_dot_exporter(self, sample_codenav):
        """Test getting GraphViz exporter with 'dot' alias."""
        exporter = get_exporter("dot", sample_codenav)
        assert isinstance(exporter, GraphVizExporter)

    def test_invalid_format(self, sample_codenav):
        """Test getting exporter with invalid format."""
        with pytest.raises(ValueError):
            get_exporter("invalid", sample_codenav)


class TestCodenavWatcher:
    """Tests for the CodenavWatcher."""

    def test_watcher_initialization(self, tmp_path):
        """Test watcher initialization."""
        watcher = CodenavWatcher(str(tmp_path))
